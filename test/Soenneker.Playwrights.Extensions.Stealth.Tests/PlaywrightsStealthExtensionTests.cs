...
public sealed class PlaywrightsStealthExtensionTests : HostedUnitTest
{
    ...
    public void BuildContextHeaders_UsesAssignedUserAgentVersionForClientHints()
    {
        ...
        Dictionary<string, string> headers = StealthHeaderBuilder.BuildContextHeaders(effectiveProfile,
        ...
        headers["sec-ch-ua"].Should()
                            .Be("\"Google Chrome\";v=\"145\", \"Not.A/Brand\";v=\"8\", \"Chromium\";v=\"145\"");
        headers["sec-ch-ua-full-version"].Should().Be("\"145.0.0.0\"");
        headers["sec-ch-ua-full-version-list"].Should()
                                              .Be(
                                                  "\"Google Chrome\";v=\"145.0.0.0\", \"Not.A/Brand\";v=\"8.0.0.0\", \"Chromium\";v=\"145.0.0.0\"");
    ...
    public void BuildDocumentHeaders_RewritesClientHintsUsingAssignedUserAgentVersion()
    {
        ...
        Dictionary<string, string> headers = StealthHeaderBuilder.BuildDocumentHeaders(effectiveProfile, requestHeaders,
            "https://example.com/next", new StealthContextOptions
            {
                InjectClientHintHeaders = true
            });
    ...
    public void BuildContextOptions_OnlyAddsGeolocationPermissionWhenRandomizationIsEnabled()
    {
        ...
        MethodInfo method =
            typeof(PlaywrightsStealthExtension).Assembly.GetType(
                                                   "Soenneker.Playwrights.Extensions.Stealth.StealthContextConfigurator")
                                               !
                                               .GetMethod("BuildContextOptions",
        ...
        (defaultContextOptions.Permissions is null ||
         !defaultContextOptions.Permissions.Contains("geolocation", StringComparer.OrdinalIgnoreCase)).Should()
            .BeTrue();
        ...
        randomizedContextOptions.Permissions!.Should().Contain(item =>
            string.Equals(item, "geolocation", StringComparison.OrdinalIgnoreCase));
    }
    ...
    {
        MethodInfo? method = typeof(PlaywrightsStealthExtension).GetMethod(
            nameof(PlaywrightsStealthExtension.LaunchStealthChromium),
    ...
    public async ValueTask NavigateToWebsite_WithStealth(CancellationToken cancellationToken)
    {
        ...
        await using IBrowser browser =
            await playwright.LaunchStealthChromium(new BrowserTypeLaunchOptions { Headless = false });

        var stealthContextOptions = new StealthContextOptions();

        await using IBrowserContext context =
            await browser.CreateStealthContext(new BrowserNewContextOptions(), stealthContextOptions);
